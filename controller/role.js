...
controller.getAll = async function(req, res) {
    try {
        await role.findAll({
            attributes: ['id_role', 'role_sname', 'role_lname', 'role_description', 'datetime_created', 'datetime_edited']
        })
        .then((data) => {
...
controller.insertData = async function(req, res) {
    ...
    const data = {
        id_role: uuidv4(),
        role_sname: dataInsert.role_sname,
        role_lname: dataInsert.role_lname,
        role_description: dataInsert.role_description,
    ...
    if (!resultErrors.isEmpty()) {
        ...
        res.status(422).json({
            "insertValue": {
                "role_sname": `${data.role_sname}`,
                "role_lname": `${data.role_lname}`,
...
controller.update = async function(req, res) {
    ...
    // input valid
    try {
        await role.update(data, { where: { id_role: `${id}` } })
        .then(([affectedRows, result]) => {
            if (affectedRows >= 1) { // berhasil
                ...
                console.log(`Berhasil mengubah data. Id: ${data.id_role}`);
            } else { // gagal
                res.status(422).json({
